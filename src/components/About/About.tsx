...
export function About() {
    ...
    return (
        <div style={{ background: "#1f2538", color: "#00d9ff" }} className="min-h-screen flex flex-col md:flex-row items-center justify-center p-6">
            {/* Contenedor de texto y contenido */}
            <motion.div
            ...
            >
                {/* Título principal */}
                {/* <h1 className="mb-6 text-3xl md:text-4xl text-center font-extrabold text-white">
                    Creando{" "}
                ...
                <div className="pt-28 md:pt-36"> {/* Ajusta este padding según necesites */}
                    <h1 className="mb-6 text-3xl md:text-4xl text-center font-extrabold text-white">
                        Creando{" "}
                        <span style={{ color: accentColor, textShadow: "2px 2px 4px rgba(0, 0, 0, 0.5)" }}>
                            websites
                ...
                <div className="flex flex-col mt-10 mb-3 md:flex-row justify-evenly md:mt-10 md:mb-5">
                    {dataAbout.map((dataText, itemIndex) => (
                        <div
                            key={dataText.id}
                            className={`cursor-pointer ${index === dataText.id
                                ? "btnClassBlue duration-300 transition-all"
                                : "border-white"
                                } md:text-lg relative px-3 md:px-8 py-4 border-2 rounded-xl flex justify-between items-center my-3 hover:bg-sky-500/10 transition-colors`}
                            onClick={() => setIndex(itemIndex)}
                        >
                            <p className="mr-4 text-md md:text-lg">{dataText.text}</p>
                            {index === dataText.id ? <BiDownArrow /> : <BiLeftArrow />}
                        </div>
                    ))}
                </div>
                ...
                {index !== 0 && dataAboutList[index - 1] && (
                    <div className="max-w-4xl p-6 mx-auto bg-sky-400/10 rounded-xl w-full">
                        <ul className="list-none pl-0">
                            {(dataAboutList[index - 1].experience || dataAboutList[index - 1].studies)?.map(
                                (item, innerIndex) => (
                                    <li key={innerIndex} className="mb-6 relative flex items-center">
                                        <div
                                            className={`bg-${accentColor} h-6 w-6 rounded-full mr-4`}
                                        ></div>
